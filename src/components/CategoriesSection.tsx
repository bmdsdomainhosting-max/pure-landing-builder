...
const CategoriesSection = () => {
  const { siteData, updateCategory } = useAdmin();
      ...
      <div className="container mx-auto px-4">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-8 gap-4">
          {siteData.categories.map((cat) => (
            ...
            >
              <EditableText value={cat.icon} onSave={(v) => updateCategory(cat.id, { icon: v })} className="text-4xl group-hover:scale-110 transition-transform duration-300" />
              <EditableText value={cat.name} onSave={(v) => updateCategory(cat.id, { name: v })} className="text-sm font-semibold text-foreground" />
            </div>
          ))}
        </div>
      </div>
